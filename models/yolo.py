    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.nkpt is None or self.nkpt == 0:
                ...
                x[i] = x[i].view(bs, self.na, self.no_det, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            else:
                # FIX: Reshape before concatenating to align anchor channels
                det = self.m[i](x[i])
                kpt = self.m_kpt[i](x[i])
                bs, _, ny, nx = det.shape
                det = det.view(bs, self.na, self.no_det, ny, nx)
                kpt = kpt.view(bs, self.na, self.no_kpt, ny, nx)
                x[i] = torch.cat((det, kpt), dim=2).permute(0, 1, 3, 4, 2).contiguous()
            ...
            if not self.training:  
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        x_kpt[..., 0::3] = (x_kpt[..., 0::3] * 2. - 0.5 + kpt_grid_x.repeat(1,1,1,1,self.nkpt)) * self.stride[i]
                        x_kpt[..., 1::3] = (x_kpt[..., 1::3] * 2. - 0.5 + kpt_grid_y.repeat(1,1,1,1,self.nkpt)) * self.stride[i]
                        x_kpt[..., 2::3] = x_kpt[..., 2::3].sigmoid()

                    y = torch.cat((xy, wh, y[..., 4:self.no_det], x_kpt), dim=-1)
