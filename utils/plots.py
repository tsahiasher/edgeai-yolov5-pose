...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for i in range(num_kpts):
        x = int(kpts[i * steps])
        y = int(kpts[i * steps + 1])
        if steps == 3:
            conf = kpts[i * steps + 2]
            if conf < 0.5:
                continue
        if x % 640 == 0 or y % 640 == 0:
            continue
        cv2.circle(im, (x, y), radius, (0, 255, 0), -1)

    if num_kpts == 4:
        skeleton = [(0, 1), (1, 2), (2, 3), (3, 0)]
        for i, j in skeleton:
            x1 = int(kpts[i * steps])
            y1 = int(kpts[i * steps + 1])
            x2 = int(kpts[j * steps])
            y2 = int(kpts[j * steps + 1])
            ...
            if x1 % 640 == 0 or y1 % 640 == 0 or x2 % 640 == 0 or y2 % 640 == 0:
                continue
